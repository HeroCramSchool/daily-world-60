...
- 価格主導の市況（2026-09-25 追加）: $136 OIL: POLAND HUNTS FOR CRUDE=10 / $1.1M A DAY: TANKER RATES RECORD=4 / $97 OIL: IRAN-US CLASHES ESCALATE=7 / OIL HITS 6-WEEK HIGH=17 / TANKER INSURANCE HITS 10%=50 / OZON SHARES CRASH 29%=10
...
- **Index 2 = その日2番目に強い「攻撃・被弾」の出来事**: index 1 と**別の出来事**（別の戦域か別のアクター）であること。hookText は「数量＋攻撃動詞＋場所」型（例: 129 DRONES HIT ODESA OVERNIGHT / 7M BARRELS: SAUDI PIPELINE HIT / 2 IRAN BOATS SUNK AT HORMUZ）。**市況アングル（価格・レート・保険料・株価・備蓄）は使わない**（2026-09-25 実測: index 2 の価格主導の見出しは中央値24回・ハズレ率75%、攻撃・被弾ネタは中央値174回。同日内で軍事>市況が22日中18日）。攻撃・被弾の出来事が2本見つからない日は、市況で埋めずに index 2 を省いて2本で保存してよい。
- **Index 3 = 勝ちレーンのトレンド枚**: 直近～48h で話題のもの。トレンドでも死ぬレーン（スーダン/コンゴ/非欧米災害/議会決議）は選ばない。

強い新展開が勝ちレーンに3本分ない日は、死ぬレーンで埋めるより**勝ちレーンの別局面（別の場所/別の指標/背景解説）で埋める**。ただし index 2 だけは市況で埋めず、攻撃・被弾の出来事が無ければ省いてよい。
...
`winning-patterns.md`（パイプラインが公開7日時点の実測で自動生成）があれば読んでタイブレークに使う。上記の勝ち/死ぬレーンと矛盾する場合はこの文書のレーン定義を優先。データ不足/2日以上古いなら無視。ただし同文書の「避ける切り口」節にある切り口は index 2 に使わない（タイブレークより一段強い扱い。ハードゲートは据え置き）。
...
- **価格主導の hookText は禁止**: `$X OIL` / `OIL HITS X` / `TANKER RATES` / `INSURANCE` / `SHARES` / `DIESEL` / `RESERVE` を hookText の先頭や主語にしない（3本すべてに適用）。石油・海運は「攻撃で止まった物理量（バレル・隻数・トン・パイプライン名）」が主語のときだけ採用する。
...
### Rank（全 story 必須・2026-09-25 追加）

各 story に `rank`（1〜3、その日の強さ順、重複なし）と `rankReason`（英語1文）を付ける。**rank は index と独立**（index 1 の地図枚が rank 1 とは限らない。習慣で地図枚を常に rank 1 にしない）。強さの基準（上から優先）: ①能動的な攻撃・被弾で新しい数字（機数・隻数・km・トン・バレル）と地名がある ②48時間以内の出来事 ③欧米の視聴者に近い利害（NATO 境界・米軍・石油の物理供給） ④歴史的な初・記録。当面は投稿順を変えず、台帳で「rank 1 がその日の最多再生になる率」を測るだけに使う。

## Step 4 — Save to Drive

`publish-results-YYYY-MM-DD.json` を "Daily World 60" に create or overwrite (UTC date)。同名は fileId を update。

### Required schema (both `stories` arrays have 3 items, index 1-3; 2 items only when no second attack event exists):
    ...
    "stories": [
      {
        ...
        "rank": 1,
        "rankReason": "New strike with hard numbers (3 tankers, a fifth of Gulf exports) inside 24h"
      }
...
- `scriptEn.stories.length` は 3（攻撃・被弾ネタが2本無い日のみ 2）AND `scriptJp.stories.length` は同数。
- **Index 2 検証（新）**: index 2 が攻撃・被弾の出来事で、index 1 と別の出来事か。3本のどれも hookText が価格主導（$X OIL / RATES / INSURANCE / SHARES / DIESEL / RESERVE）でないか。
- **Rank 検証（新）**: 全 story に `rank`（1〜3、重複なし）と `rankReason`。
...
完了時: `{ "date": "YYYY-MM-DD", "status": "ok", "driveFileId": "...", "storyCount": 3, "lanes": ["iran-gulf-oil","russia-ukraine","iran-gulf-oil"], "deadLaneCount": 0, "mapLane": true, "mapMarkerCount": 3, "researchSources": 7, "sportsCount": 0, "patternsApplied": true, "hookPatterns": {"number": 3}, "ranks": [1, 3, 2], "excludedDuplicates": 2, "sources": ["Reuters", "BBC"] }`
失敗時: `{ "date": "YYYY-MM-DD", "status": "failed", "error": "原因" }`

## Constraints

- 予算: 18分以内 / $1.50以内。動画生成・TTS・SNS投稿はしない。
- pipeline がフォーマット依存。schema 厳守（3 stories を原則・index1=map・index2=別の出来事の攻撃・被弾ネタ・価格主導の hookText なし・全storyに rank/rankReason・beatVisuals+heroMotion・勝ちレーン集中・共食いなし）。
